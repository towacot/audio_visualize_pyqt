...
class Spectrogram:
    ...
    def calc(self,spectrogramqueue):
        ...
        if self.data_is_not_enough:
            ...
            specs = np.abs(self.fft(self.process_data))**2
            specs = librosa.power_to_db(specs, ref=np.max)
            spectrogramqueue.put(specs)
            #スペクトログラムをGUIに渡す
            
    
    def run(self,playstate,spectrogramQueue):
        while True:
            if not playstate.empty():
                print("calc run")
                print(self.testo)
                self.calc(spectrogramQueue)
            else:
                time.sleep(0.01)
